...
import {IWeirV2LaunchFactoryGraduation} from "./interfaces/ILaunchpadV2Graduation.sol";

/**
 * @title WeirV2BondingCurve
 ...
 */
contract WeirV2BondingCurve is ReentrancyGuard {
     ...
     * @param factory_ WeirV2LaunchFactory address, the only caller allowed through `onlyFactory`.
    ...
    constructor(
        ...
        deployer = deployer_;
        // Passed explicitly rather than read from msg.sender: WeirV2LaunchFactory
        // deploys this curve indirectly through WeirV2LaunchDeployer to keep its
    ...
    /**
     * @notice Tradeable quote reserve only, matching IWeirV2BondingCurve.
     */
    ...
    /**
     * @notice Tradeable token reserve only, matching IWeirV2BondingCurve.
     */
        ...
        returns (uint256 quoteOut)
    {
        ...
        uint256 grossQuoteOut = WeirV2BondingCurveMath.getAmountOut(tokensIn, tokenReserveBefore, quoteReserveBefore, 0);
    ...
    function _tryAutoGraduate() private {
        if (readyToGraduate()) {
            try IWeirV2LaunchFactoryGraduation(factory).graduate(token) {}
            catch {
    ...
    function _sweepFees(uint256 minBuybackTokensOut, bool executeBuyback) private {
        ...
        if (buybackAmount != 0) {
            ...
            if (reserveMovementBps <= maxInternalPriceImpactBps) {
                ...
                uint256 tokensOut =
                    WeirV2BondingCurveMath.quoteAmountOut(buybackAmount, quoteReserve_, tokenReserve_, 0);
     ...
     * WeirV2FeeEscrow, and a permissioned quote asset can stop delivering to
     ...
     *
     * Mirrors WeirV2MemeHook.rescuePoolFees for the post-graduation pool and
     * WeirV2LaunchFactory.rescueSweptGraduation for the reserves in between.
     */
